...
# TODO:
#  - Change filename pattern support to what in AutoMuxer
#  - Multi-thread support. Probably a dedicated thread for each hash type

import sys, os, zlib, hashlib, fnmatch, shutil, re, time, struct, multiprocessing, math
...
def hasher_s(fileName):
	iHash, md5, sha1, sha224, sha256, sha384, sha512, ed2k, error = hasher(fileName)
...
def processFile(fileName, fromFolder = False):
	...
	sHash, md5, sha1, sha224, sha256, sha384, sha512, ed2k, error = hasher_s(fileName)
	newName = fileName
...
# Parse paramenters
def parseParams():
	global pathList, addcrc, createsfv, sfvPath, force, recursive, searchSubFolder, showChecksumResult
	global enableMd5, enableSha1, enableSha224, enableSha256, enableSha384, enableSha512, enableEd2k
	...
	while i < len(sys.argv):
		arg = sys.argv[i]
		if arg.startswith('-') and not treatAllAsFilenames:
			if arg.startswith('--'):
				arg = arg[2:]
			else:
				arg = arg[1:]
...
def printReadme():
	...
	print("  --most                          Enable CRC-32, MD5, SHA-1, SHA-256, SHA-512, and ED2K.")
	print("  --all                           Enable all supported hashes.")
	print("  --inputs                        Treat all remaining paramenters as filenames.\n")
	print("  Currently supported hash types: MD5, SHA-1, SHA-224, SHA-256, SHA-384, SHA-512, ED2K.")
